...
base_dir = "/Users/haseeb.rehman/Desktop/For_Animation/4th_Year/Miscs/Trier_uni_meeting"
...
bar_width = 0.10
colors = {
    'no_da': '#A52A2A', # muted red for No DA
...
for i, var in enumerate(variables_met):
    if var in agg_metrics_general.index:
        ...
        bars1 = axes[i].bar(x + 0 * (bar_width + 0.01), before_values, bar_width, label='No DA', color=colors['no_da'], alpha=0.7, edgecolor='black')
...
bars1 = ax.bar(x + 0 * (bar_width + 0.01),
               ...
               bar_width, label='No DA', color=colors['no_da'], alpha=0.7, edgecolor='black')
...
bars1 = ax_pod_far.bar(x_podfar + 0 * (bar_width + 0.01), podfar_values_before, bar_width, label='No DA', color=colors['no_da'], alpha=0.7, edgecolor='black')
...
bars_cv5 = ax4.bar(x + bar_width + 0.01, improv_cv5_precip, bar_width, label='CV5', color=colors['after_cv5'], edgecolor='black')
...
for bar, val in zip(bars_cv5, improv_cv5_precip):
    if np.isfinite(val):
        label = f'{val:+.1f}%' if val >= 0 else f'{val:.1f}%'
        ax4.text(bar.get_x() + bar.get_width()/2, val/2, label, ha='center', va='center', rotation=90, color='black', fontsize=8)
ax4.set_xticks(x + (bar_width + 0.01) / 2)
...
bars_cv5 = ax5.bar(x + bar_width + 0.01, improv_cv5_temp, bar_width, label='CV5', color=colors['after_cv5'], edgecolor='black')
...
for bar, val in zip(bars_cv5, improv_cv5_temp):
    if np.isfinite(val):
        label = f'{val:+.1f}%' if val >= 0 else f'{val:.1f}%'
        ax5.text(bar.get_x() + bar.get_width()/2, val/2, label, ha='center', va='center', rotation=90, color='black', fontsize=8)
ax5.set_xticks(x + (bar_width + 0.01) / 2)
