...
# Define folders
base_path = "/Users/haseeb.rehman/Desktop/For_Animation/3rd_Year/1_month_simulation_2021_new_GFS_000_cv5"
...
new_folder = "/Users/haseeb.rehman/Desktop/For_Animation/3rd_Year/1_month_simulation_2021_new_GFS_000/After_DA"
...
for gpm_file in os.listdir(gpm_folder):
    if gpm_file.endswith(".png") and "GPM_IMERG" in gpm_file:
        try:
            ...
            dt = datetime.strptime(raw_date, "%Y_%m_%d_%H%M")
            wrf_timestamp = dt.strftime("%Y-%m-%d_%H_%M_00")
        ...
        new_path = os.path.join(new_folder, wrf_filename)

        if not os.path.exists(before_path) or not os.path.exists(after_path) or not os.path.exists(new_path):
        ...
        img_new = manual_crop(mpimg.imread(new_path), top=140, bottom=20, left=40, right=50)

        # Plot
        fig = plt.figure(figsize=(8, 8))
        ax1 = fig.add_axes([0.05, 0.55, 0.45, 0.4])
        ax2 = fig.add_axes([0.55, 0.55, 0.45, 0.4])
        ax3 = fig.add_axes([0.05, 0.25, 0.45, 0.4])
        ax4 = fig.add_axes([0.55, 0.25, 0.45, 0.4])
        ax1.imshow(img_gpm)
        ...
        ax3.text(0.5, -0.08, "(c) WRF (After DA CV3)", fontsize=10, color='black', ha='center', va='top', transform=ax3.transAxes)
        ax4.text(0.5, -0.08, "(d) WRF (After DA CV5)", fontsize=10, color='black', ha='center', va='top', transform=ax4.transAxes)
