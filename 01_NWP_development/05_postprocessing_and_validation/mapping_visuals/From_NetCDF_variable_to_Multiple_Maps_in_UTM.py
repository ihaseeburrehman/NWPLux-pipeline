...
path = "/Users/haseeb.rehman/WRF/WRFV4.5/run/2018_June_Lux_6hr"
...
for filename in os.listdir(path):
    ...
    if nc_dataset is not None:
        ...
        highest_RAINSH_value = RAINSH_data[index_of_highest_value]
        sum_at_highest_location = highest_RAINC_value + highest_RAINNC_value + highest_RAINSH_value
        ...
        df = pd.read_excel("/Users/haseeb.rehman/Desktop/4_Asta_Stations.xlsx")
        
        # Extract the lat, lon and precipitation columns from the dataframe
        lats = df["Latitude"]
        lons = df["Longitude"]
        stationname = df["Stationsname"]
        ...
        for x, y, label in zip(easting, northing, stationname):
            plt.text(x, y, label, fontsize=5, ha='center', va='top' , color="cyan" , alpha=0.7)
        ...
        plt.title(str(sum_data), fontsize=8, color='grey')
