...
fig, axes = plt.subplots(nrow, ncol, figsize=(16, 13),
...
for r, (idx, tlab) in enumerate(TIMES):
    for c, (m, mlab) in enumerate(MODELS):
        ...
        ax.imshow(np.ma.masked_invalid(base), extent=EXTENT, origin="upper",
                  cmap=ListedColormap(["#b8c4c8"]), vmin=0, vmax=1, zorder=1)
        # flooded depth
        ...
        if riv is not None:
            riv.plot(ax=ax, color="#08306b", linewidth=0.6, alpha=0.9, zorder=3)
